...
GRBC_all_tax <- merge(GRBC_Freshwater2016_tax, GRBC_Silva_132_tax, all = TRUE) %>%
  ...
  mutate(time = ifelse(str_detect(otu, "_D31"), "D31", "D3"),
         color = 
          ...
          ifelse(host_species == "Monoraphidium", "#FFD400",
...
#Matches the color of each species in the graph below
...
agg_genus_data_D3_color %>% 
  ...
  theme(axis.title.x = element_blank(),
    axis.text.x = element_text(angle = 70, hjust = 1, size = 10, color = color_chlorella$color),
    axis.title.y = element_text(size = 10),
    axis.text.y = element_text(size = 10)) +
  #facet_wrap(~host_species, scales = "free_y", dir = "v") +
  scale_fill_manual(values = c("#7FC97F", "#BEAED4","#E0115F","#FFD400","#386CB0")) +
  guides(fill = guide_legend(size = 10)) +
  ...
  scale_y_continuous("Average Relative Abundance") +
   scale_x_reordered()
df <- agg_genus_data_D3_color

df %>%
  ggplot( aes(x = fct_inorder(genus, avg_rel_abund), y = avg_rel_abund, fill = host_species))+
  geom_col() +
  geom_text(aes(label = host_species, y = Inf), vjust = 1, size = 3)+
  scale_fill_manual(values = c("#7FC97F", "#BEAED4","#E0115F","#FFD400","#386CB0"))+
  theme(axis.title.x = element_blank(),
    axis.text.x = element_text(angle = 70, hjust = 1, size = 10, color = df$color),
    axis.title.y = element_text(size = 10),
    axis.text.y = element_text(size = 10)) +
  guides(fill = guide_legend(size = 10)) +
  labs( y = "Relative Abundance (Genus)", title = "Day 3 Genera Present in Host Phycospheres", fill = "Host Species") +
