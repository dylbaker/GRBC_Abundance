...
#Detecting which Phycosphere_otus match up to isolates
if_host_match <- ifelse(agg_genus_data_D3$genus %in% Phycosphere_GRBC_D3$genus & str_detect(Phycosphere_GRBC_D3$host_species,"Chlorella") , "red", 
                        ifelse(agg_genus_data_D3$genus %in% Phycosphere_GRBC_D3$genus & str_detect(Phycosphere_GRBC_D3$host_species,"Coelastrum"), "#BEAED4",
                        ifelse(agg_genus_data_D3$genus == Phycosphere_GRBC_D3$genus & str_detect(Phycosphere_GRBC_D3$host_species,"Scenedesmus"), "#FDC086",
                        ifelse(agg_genus_data_D3$genus == Phycosphere_GRBC_D3$genus & str_detect(Phycosphere_GRBC_D3$host_species,"Monoraphidium"), "#FFFF99",
                        ifelse(agg_genus_data_D3$genus == Phycosphere_GRBC_D3$genus & str_detect(Phycosphere_GRBC_D3$host_species,"Selenastrum"), "#386CB0", "black"))))) #Matches the color of each species in the graph below
...
agg_genus_data_D3 %>% 
  ...
  ungroup() %>% 
  mutate(host_species = factor(host_species, levels = c("Chlorella", "Coelastrum", "Scenedesmus", "Monoraphidium", "Selenastrum")), genus = reorder_within(genus, -avg_rel_abund, host_species)) %>%
  mutate(color = ifelse(str_detect(genus, paste0(Phycosphere_GRBC_D3$genus)) & str_detect(Phycosphere_GRBC_D3$host_species,"Chlorella") , "#7FC97F", 
                        ifelse(str_detect(genus, paste0(Phycosphere_GRBC_D3$genus)) & str_detect(Phycosphere_GRBC_D3$host_species,"Coelastrum"), "#BEAED4",
                        ifelse(str_detect(genus, paste0(Phycosphere_GRBC_D3$genus)) & str_detect(Phycosphere_GRBC_D3$host_species,"Scenedesmus"), "#FDC086",
                        ifelse(str_detect(genus, paste0(Phycosphere_GRBC_D3$genus)) & str_detect(Phycosphere_GRBC_D3$host_species,"Monoraphidium"), "#FFFF99",
                        ifelse(str_detect(genus, paste0(Phycosphere_GRBC_D3$genus)) & str_detect(Phycosphere_GRBC_D3$host_species,"Selenastrum"), "#386CB0", "black")))))) %>%
  ...
  theme(axis.title.x = element_blank(),
    ...
    axis.title.y = element_text(size = 9),
    axis.text.y = element_text(size = 7, color = agg_genus_data_D3$color)) +
  guides(fill = guide_legend(size = 10)) +
  labs( y = "Relative Abundance (Genus)", title = "Day 3 Genera Present in Host Phycospheres", fill = "Host Species") +
...
agg_genus_data_D31 %>% 
  #Filter out reads below the detection limit that are not part of our isolate collection
  filter(!(agg_rel_abund < 1e-3) | genus %in% Phycosphere_GRBC_D31$genus) %>%
  group_by(genus, host_species) %>%
  summarize(avg_rel_abund = mean(agg_rel_abund)) %>% #Take average of all the groups for one host species
  ungroup() %>% 
  mutate(host_species = factor(host_species, levels = c("Chlorella", "Coelastrum", "Scenedesmus", "Monoraphidium", "Selenastrum")), genus = reorder_within(genus, -avg_rel_abund, host_species)) %>%
  ...
  theme(axis.title.x = element_blank(), 
    ...
    axis.title.y = element_text(size = 9))+
  guides(fill = guide_legend(size = 10)) +
  labs( y = "Relative Abundance (Genus)", title = "Day 31 Genera Present in Host Phycospheres", fill = "Host Species") +
  scale_y_continuous("Average Relative Abundance") +
  coord_flip()+
  scale_x_reordered() +
  scale_y_continuous()
...
ggsave(filename = "GRBC Abundance in Phycosphere.png",  dpi = 300, path = "./figures", height = 8, width = 8)
