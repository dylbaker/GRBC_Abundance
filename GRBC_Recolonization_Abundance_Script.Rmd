...
GRBC_all_tax <- merge(GRBC_Freshwater2016_tax, GRBC_Silva_132_tax, all = TRUE) %>%
  ...
  mutate(triplicate = ifelse(str_detect(jar, paste(jar_num$jar_no[1:3], collapse = "|")), 1,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[4:6], collapse = "|")), 2,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[7:9], collapse = "|")), 3,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[10:12], collapse = "|")), 4,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[13:15], collapse = "|")), 5,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[16:18], collapse = "|")), 6,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[19:21], collapse = "|")), 7,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[22:24], collapse = "|")), 8,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[25:27], collapse = "|")), 9,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[28:30], collapse = "|")), 10,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[31:33], collapse = "|")), 11,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[34:36], collapse = "|")), 12,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[37:39], collapse = "|")), 13,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[40:42], collapse = "|")), 14,
                        ifelse(str_detect(jar, paste(jar_num$jar_no[43:45], collapse = "|")), 15, "NA")))))))))))))))) %>%
  #Rearrage columns in a readable order
  select(otu, jar,triplicate, time, color, everything())
...
plot_D3_3um <- agg_genus_data_D3_3um %>% filter (genus %in% top_genera_D3 | genus %in% top_genera_D31) %>%
...
agg_genus_data_D3_color <- agg_genus_data_D3 %>% 
  ...
  mutate(time = "D3")  %>% 
  mutate(host_species = factor(host_species, levels = c("Chlorella", "Coelastrum", "Scenedesmus", "Monoraphidium", "Selenastrum")), genus = reorder_within(genus, -avg_rel_abund, host_species))
...
sce_D3 <- agg_genus_data_D3_color %>% 
  ...
  geom_bar(stat = "Identity") +
  theme(axis.title.x = element_blank(),
    axis.text.x = element_text(angle = 70, hjust = 1, size = 9, color = color_scenedesmus$color),
    axis.title.y = element_text(size = 10),
    axis.text.y = element_text(size = 10)) +
...
p <- ggarrange(chl_D3 + font("xlab", size = 10),
          coe_D3 + rremove("ylab") + font("xlab", size = 10), 
          sce_D3 + rremove("ylab") + font("xlab", size = 10), 
          mon_D3 + rremove("ylab") + font("xlab", size = 10), 
          sel_D3 + rremove("ylab") + font("xlab", size = 10),
          chl_D31 + font("xlab", size = 10),
          coe_D31 + rremove("ylab") + font("xlab", size = 10), 
          sce_D31 + rremove("ylab") + font("xlab", size = 10), 
          mon_D31 + rremove("ylab") + font("xlab", size = 10), 
          sel_D31 + rremove("ylab") + font("xlab", size = 10),
          legend = "bottom", nrow = 2, ncol = 5, labels = "AUTO", vjust = 1)
annotate_figure(p, top = "Day 3 and Day 31 Isolates Present in Host Phycospheres")
ggsave(filename = "Isolates D3-D31 Phycosphere Comparison.png",  dpi = 275, path = "./figures", width = 22, height = 14)
...
column_plot_D3 <- Phycosphere_GRBC_D3  %>%
  ...
  theme(axis.title.x = element_blank(), 
    ...
    plot.title = element_text(size = 14))+
  guides(fill = guide_legend(size =10)) +
  labs( x = "Genus", y = "Average Relative Abundance (Genus)", title = "Day 3 GRBC Isolates Present in Algal Phycospheres by Abundance", fill = "Host Species")  +
  scale_y_continuous("Average Relative Abundance", limits = c(0, 0.16), breaks = c(0.0, 0.05, 0.1, 0.15))
...
mutate(host_species = factor(host_species, levels = c("Chlorella", "Coelastrum", "Scenedesmus", "Monoraphidium", "Selenastrum"))) %>%
  ...
  theme(axis.title.x = element_blank(), 
    ...
    plot.title = element_text(size = 14))+
  guides(fill = guide_legend(size =10)) +
  labs( x = "Genus", y = "Average Relative Abundance (Genus)", title = "Day 31 GRBC Isolates Present in Algal Phycospheres by Abundance", fill = "Host Species") +
  scale_y_continuous("Average Relative Abundance", limits = c(0, 0.16), breaks = c(0.0, 0.05, 0.1, 0.15)) 

ggarrange(column_plot_D3, column_plot_D31 + font("xlab", size = 10),
          common.legend = TRUE, legend = "right", nrow = 2, ncol = 1, labels = "AUTO", align = "v", heights = c(1.7,1), vjust = 1.5)
ggsave(filename = "GRBC Abundance in Phycosphere.png",  dpi = 300, path = "./figures", height = 9, width = 8)
...
Plot_GRBC_D3 <- Phycosphere_GRBC_D3 %>%
arrange(genus, desc(avg_rel_abund)) %>%
  mutate(host_species = factor(host_species, levels = c("Chlorella", "Coelastrum", "Scenedesmus", "Monoraphidium", "Selenastrum"))) %>%
#mutate(triplicate = fct_reorder(triplicate, as.numeric(triplicate))) %>%
ggplot(aes(x = host_species, y = avg_rel_abund, fill = genus)) + 
  ...
  theme(axis.title.x = element_blank(), 
    ...
    plot.title = element_text(size = 14))+
  guides(fill = guide_legend(size =10)) +
  labs( x = "Host Species", y = "Average Relative Abundance (Genus)", title = "Day 3 GRBC Isolates Present in Host Algal Phycospheres", fill = "Genus")

ggarrange(plot_D3 + font("xlab", size = 10),Plot_GRBC_D3 + font("xlab", size = 10),  plot_D31 + rremove("ylab") + font("xlab", size = 10),
          common.legend = TRUE, legend = "bottom", nrow = 1, ncol = 3, labels = "AUTO")
...
ggplot(aes(x = host_species, y = avg_rel_abund, fill = genus)) + 
  ...
  theme(axis.title.x = element_blank(), 
    ...
    plot.title = element_text(size = 14))+
  guides(fill = guide_legend(size =10)) +
  labs( x = "Host Species", y = "Average Relative Abundance (Genus)", title = "Day 31 GRBC Isolates Present in Host Algal Phycospheres", fill = "Genus")
...
agg_genus_data_D3 %>%
		group_by(triplicate, genus, host_species) %>%
		summarize(avg_rel_abund = mean(agg_rel_abund)) %>%
		arrange(desc(avg_rel_abund)) %>%
  ungroup()
