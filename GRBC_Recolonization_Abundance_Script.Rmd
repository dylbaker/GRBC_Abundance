...
agg_genus_data_D3_color %>%
  ...
  theme(axis.title.x = element_blank(),
    ...
    axis.text.y = element_textangle = 70, hjust = 1, size = 10, color = agg_genus_data_D3_color$color) +
  guides(fill = guide_legend(size = 10)) +
...
df %>%
  ggplot( aes(x = fct_inorder(genus, host_species), y = avg_rel_abund, fill = host_species)) +
  geom_col() +
  scale_fill_manual(values = c("#7FC97F", "#BEAED4","#E0115F","#FFD400","#386CB0"))+
