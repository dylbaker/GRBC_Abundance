...
GRBC_all_tax <- merge(GRBC_Freshwater2016_tax, GRBC_Silva_132_tax, all = TRUE) %>%
  ...
  mutate(time = ifelse(str_detect(otu, "_D31"), "D31", "D3"),
         color = 
          ...
          ifelse(host_species == "Scenedesmus", "#FDC086",
          ifelse(host_species == "Monoraphidium", "#FFFF99",
          ifelse(host_species == "Selenastrum", "#386CB0", "black")))))) %>%
  #Rearrage columns in a readable order
  select(otu, jar, time, color, everything())
...
Phycosphere_GRBC_D3 <- inner_join(GRBC_all_tax, agg_genus_data_D3) %>%
  ...
  summarize(avg_rel_abund = mean(agg_rel_abund)) %>% #Take average of all the groups for one host species
  ungroup() %>%
  select(genus, host_species, time, color)
...
agg_genus_data_D3_color$color[is.na(agg_genus_data_D3_color$color)]  <- "black"
...
agg_genus_data_D3_color <- agg_genus_data_D3 %>% 
  ...
  summarize(avg_rel_abund = mean(agg_rel_abund)) %>% #Take average of all the groups for one host species
  ungroup() %>% 
  mutate(host_species = factor(host_species, levels = c("Chlorella", "Coelastrum", "Scenedesmus", "Monoraphidium", "Selenastrum")), genus = reorder_within(genus, -avg_rel_abund, host_species), color = "black")
...
agg_genus_data_D3_color <- agg_genus_data_D3 %>% 
  ...
  ggplot(agg_genus_data_D3_color ,aes(x = genus, y = avg_rel_abund, fill = host_species)) +
  geom_bar(stat = "Identity") +
  facet_grid(cols = vars(host_species), scales = "free") +
  #facet_wrap(~host_species, scales = "free_y", dir = "v") +
  scale_fill_brewer(type = "qual", palette = 1) +
  theme(axis.title.x = element_blank(),
    axis.text.x = element_text(angle = 70, hjust = 1, size = 10, color = agg_genus_data_D3_color$color),
