...
```{r Phycosphere OTU D3/D31 Comparison by Host Species}
...
ggplot(aes(x = host_species, y = agg_rel_abund, fill = genus)) + 
  ...
  labs( y = "Relative Abundance (Genus)", subtitle = "D3 Genus Abundance by Host Species", fill = "Genus")
...
ggplot(aes(x = host_species, y = agg_rel_abund, fill = genus)) + 
  ...
  theme(axis.title.x = element_blank(), 
    ...
    axis.title.y = element_text(size = 9))+
  guides(fill = guide_legend(size = 10)) +
  labs( y = "Relative Abundance (Genus)", subtitle = "D31 Genus Abundance by Host Species", fill = "Genus")
...
#Graph D3 and D31 genuses that are present in host phycospheres (not complete yet due to incomplete sequencing)
Phycosphere_GRBC_D3 %>%
  filter(avg_rel_abund != 0) %>%
  group_by(host_species) %>%
  mutate(avg_rel_abund = mean(avg_rel_abund)) %>%
  ungroup() %>%
...
Phycosphere_GRBC_D3  %>%
  mutate(otu = fct_reorder(host_species, avg_rel_abund, .fun = 'median', .desc = TRUE)) %>%
  ggplot( aes(x = genus, y = avg_rel_abund, fill = otu)) +
geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = colors) +
  theme(axis.title.x = element_blank(), 
...
Phycosphere_GRBC_D31  %>%
  mutate(otu = fct_reorder(otu, avg_rel_abund, .fun = 'median', .desc = TRUE)) %>%
  ggplot( aes(x = genus, y = avg_rel_abund, fill = otu)) +
  scale_fill_manual(values = colors) +
  geom_col() +
  xlab("Isolate") +
...
column_plot_D3 <- Phycosphere_GRBC_D3  %>%
  ...
  ungroup() %>%
mutate(host_species = factor(host_species, levels = c("Chlorella", "Coelastrum", "Scenedesmus", "Monoraphidium", "Selenastrum"))) %>%
  mutate(genus = fct_reorder(genus, avg_rel_abund, .fun = 'mean', .desc = TRUE)) %>%
  ggplot( aes(x = genus, y = avg_rel_abund, fill = host_species)) +
...
# ggarrange(plot_3, plot_4 + rremove("ylab"),
#           common.legend = TRUE, legend = "bottom", nrow = 2, ncol = 1, labels = "AUTO", heights = c(1.5,1), align = "v")


# Plot 4: Stacked Barplot Genus level Sample Type 
