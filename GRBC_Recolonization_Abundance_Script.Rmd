...
jar_otu <- anti_join(Phycosphere_otu, sampledata)

GRBC_otu <- inner_join(Phycosphere_otu, sampledata) %>%
  mutate( color = 
          ifelse(host_species == "Chlorella", "#7FC97F",
          ifelse(host_species == "Coelastrum", "#BEAED4",
          ifelse(host_species == "Scenedesmus", "#E0115F",
...
Phycosphere_otu_count <- inner_join(Phycosphere_otu, Phycosphere_counts, by = "Group") %>%
  ...
  #Add Host Species Information
    mutate(host_species = ifelse(str_detect(Group, "Chlorella"), "Chlorella",
                        ifelse(str_detect(Group, "Coelastrum"), "Coelastrum",
                        ifelse(str_detect(Group, "Scenedesmus"), "Scenedesmus",
                        ifelse(str_detect(Group, "Monoraphidium"), "Monoraphidium",
                        ifelse(str_detect(Group, "Selenastrum"), "Selenastrum",
                               "Natural Community"))))))
...
agg_genus_data_D3<- inner_join(Phycosphere_otu_D3, Phycosphere_tax) %>% #Groups by otu automatically
  group_by(genus, Group, host_species) %>% #Triplicate 1 is J1,2,3 and so on for each host species
...
plot_D3 <- agg_genus_data_D3 %>% filter (genus %in% top_genera_D3) %>%
...
ggarrange(plot_D3 + font("xlab", size = 10),
...
Phycosphere_GRBC_D3 <- inner_join(GRBC_otu, agg_genus_data_D3) %>%
  filter(!str_detect(asv, "D31")) %>%
...
#Color schemes for each host species (D3)
...
sel_D31 <- abundance_plot_isolate(species_name = "Selenastrum", data = agg_genus_data_D3_color,
                                 color_data = color_selenastrum, bar_color = "#386CB0",
                                 genus_species = "Selenastrum capricornutum")
...
ggsave(filename = "Isolates D3-D31 Phycosphere Comparison.png",  dpi = 300, path = "./figures", width = 14, height = 18)

ggsave(filename = "Isolates D3-D31 Phycosphere Comparison.pdf",  dpi = 300, path = "./figures", width = 14, height = 18)
