...
sce_D31 <- agg_genus_data_D31_color %>% 
  ...
  guides(fill = guide_legend(
    title = substitute(paste(italic("Scenedesmus acuminatus"), " Phycosphere Bacteria")),
    title.position = "top"
  )) +
  labs( y = "Relative Abundance (Genus)", x = "Genus", fill = '') +
  scale_y_continuous("Average Relative Abundance", limits = c(0, 0.3), breaks = c(0.0, 0.1, 0.2, 0.3)) +
  theme(axis.title.x = element_text(size = 16),
    ...
    legend.text = element_text(vjust = .5, size = 13
    )) +
   scale_x_reordered() +
  ggtitle(label = "Rank Abundance Curve Based on 16S v4 Tag Sequencing of Phycosphere Communities" , subtitle = "Representative Plot")
