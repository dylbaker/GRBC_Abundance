...
ggsave(filename = "Isolates D3-D31 Phycosphere Comparison.png",  dpi = 300, path = "./figures", width = 22, height = 14)
...
sig_genera_host <- genera_tests %>%
  filter(p.value.adj <= 0.05) %>%
					pull(genus)

genera_tests_time <- agg_genus_data %>%
          nest(sample_data = c(-time)) %>%
					mutate(test=map(sample_data, ~tidy(kruskal.test(agg_rel_abund~host_species, data=.)))) %>%
					unnest(test) %>%
					mutate(p.value.adj=p.adjust(p.value, method="BH")) %>%
					arrange(p.value.adj)

sig_genera_time <- genera_tests_time %>%
  filter(p.value.adj <= 0.05) %>%
					pull(genus)
